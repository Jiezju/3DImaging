...
/* ---------------------------------------------------------------- */
    ...
    Sift::extrema_detection(core::FloatImage::ConstPtr s[3], int oi, int si) {
         ...
         *
         * 储存连续内存的偏移量 ， 一个中心点，以及 在当前层 中 的 9 个点
         * */
        ...
        int detected = 0;
        int off = w;
        /* 输入 3 层 金字塔图像， 遍历中间层的 每个像素节点进行 27 个点 的最值判断 */
...
#       define AT(S, OFF) (dogs[S]->at(px + OFF))
            ...
            if (std::abs(val) < this->options.contrast_threshold
                ...
                || kp.sample<-1.0f
                             || kp.sample>(float)
                this->options.num_samples_per_octave
