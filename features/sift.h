 ...
 */
    ...
    public:
        ...
        struct Options {
            ...
            // 是 N = S + 3
            /*
             * N 表示金字塔每个 octave 中 层数 [ 每组分为 N 层 ]
             * 从 底层 到 顶层 不断经过 高斯核 进行模糊
             * */
             ...
             * 高斯金字塔分裂的组数 O = floor(log(min(height, width)) - 2)
             * 下一组的图像 由 上一组图像 按照 隔点采样 长宽减半
             */
            int max_octave;
...
/* ---------------------------------------------------------------- */
    ...
    Sift::Options::Options(void)
            : num_samples_per_octave(3), min_octave(0), max_octave(4), contrast_threshold(-1.0f),
              edge_ratio_threshold(10.0f), base_blur_sigma(1.6f), inherent_blur_sigma(0.5f), verbose_output(false),
              debug_output(false) {
    }
